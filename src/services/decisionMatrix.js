 ...
 */
export function evaluateDecisionMatrix(input) {
    ...
    if (!hasErpRegistration) {
        if (!isHighValue) {
            ...
            return {
                ...
                rationale: 'Fornecedor sem cadastro prévio no ERP e contratação de valor menor ou igual a R$ 50.000,00. Fluxo ágil com aprovação de alçada, preenchimento cadastral e integração direta.',
                suggestedSteps: [
                    {
            ...
            return {
                ...
                rationale: 'Fornecedor sem cadastro prévio no ERP com valor superior a R$ 50.000,00. Exige diligência completa: Alçada, Background Check Legal (BC Legal), formalização contratual e aceite operacional.',
                suggestedSteps: [
                    {
                        stepName: '1. Alçada de Aprovação',
        ...
        if (!isHighValue) {
            ...
            return {
                ...
                rationale: 'Fornecedor já cadastrado no ERP e contratação de até R$ 50.000,00. Fluxo mais rápido da operação: validação de alçada e liberação imediata do código existente.',
                suggestedSteps: [
                    {
                        stepName: '1. Alçada de Aprovação',
            ...
            return {
                ...
                rationale: 'Fornecedor já cadastrado no ERP, porém com valor superior a R$ 50.000,00. Requer revalidação de BC Legal e formalização do instrumento contratual ou aditivo antes do pedido.',
                suggestedSteps: [
                    {
                        stepName: '1. Alçada de Aprovação',
 ...
 */
export function buildDecisionFlow(awardId, supplierId, input) {
    const evalResult = evaluateDecisionMatrix(input);
    const now = new Date().toISOString();
    const steps = evalResult.suggestedSteps.map((step, idx) => ({
