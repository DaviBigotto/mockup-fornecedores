...
export async function seedCleanDatabase(targetPool = pool) {
    ...
    `);
        ...
        for (const [id, name, desc] of categories) {
            await client.query(`INSERT INTO categories (id, name, description) VALUES ($1, $2, $3) ON CONFLICT (name) DO NOTHING`, [id, name, desc]);
        }
        ...
        for (const [id, code, name, region, uf] of businessUnits) {
            await client.query(`INSERT INTO business_units (id, code, name, region, state_uf) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (code) DO NOTHING`, [id, code, name, region, uf]);
        }
        ...
        for (const u of demoUsers) {
            await client.query(`INSERT INTO users (id, email, name, role, company_name, password_hash) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name`, [u.id, u.email, u.name, u.role, u.companyName, '123456']);
        }
        ...
        for (const [id, code, name, desc, mandatory] of docTypes) {
            await client.query(`INSERT INTO document_types (id, code, name, description, is_mandatory) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (code) DO NOTHING`, [id, code, name, desc, mandatory]);
        }
        ...
        await client.query(`INSERT INTO supplier_status_history (
